...
We also added the new Fig.~S7 \noteg{is this order fine?} to the Supporting Information where we discuss at some length within the caption the important issues raised by the reviewer. The figure and its caption are reproduced below.
...
\rood{To correct for finite-size effects associated with the simulation of a single duplex within a periodic simulation box, we apply the fluctuation correction proposed by Ouldridge et al.\citep{Ouldridge2010ExtractingSimulations} to correct our predictions for $f_\mathrm{hybridized}$ bulk yields. We correct the values observed in our simulations $f_\mathrm{hybridized}^\mathrm{sim}$ to the bulk corrected result $f_\mathrm{hybridized}$ using the following relations:}

    \begin{equation}\label{}
    \phi = \frac{\mathrm{f_\mathrm{hybridized}^\mathrm{sim}}}{\mathrm{1 - f_\mathrm{hybridized}^\mathrm{sim}}}
    \end{equation}
...
\noteg{Have not incorporated this yet, but this would be one way to address it without making the FE calculations significantly (and unnecessarily) more complicated}
``\rood{In applying the NN models to each macrostate, we made the simplifying assumption that the ensemble of microstates constituting each macrostate could be represented by a single pattern of Watson-Crick base pairing that are schematically illustrated in Fig.~2a. Indeed, each macrostate is composed of an ensemble of partially frayed stayed with higher free energy contributions than the intact pattern. However, when looking at the difference between states, we have found that incorporating frayed contributions changes 
$\Delta$ F values by $<$ 1 kJ/mol when compared to used a single macrostate. Given the extent to which a single NN representation captures each macrostates, we have elected to maintain a single microstate free energy estimate for each macrostate}''

\noteg{Can also include a figure for the F4 or shifted states showing a minimal difference.}
...
\textbf{Author Reply:}  \noteg{I added everything below, not sure if this is adequately adressing the mistake} We apologize for the confusion and have found that it stems from a unit conversion error for the GC-core sequence. Specifically, we did not convert the $\Delta$ F (5.1 kT) from units kT to kJ/mol, which should result in a much larger $\Delta$ F (13.7 kJ/mol) relative to the hybridized state.  

We recognize that this is a significant miscalculation, and we have re-checked all free energy results to ensure it was an error for the GC-core free energy calculations only and did not affect the reported values for GC-end and AT-all. Furthermore, we have verified that the error does not propagate to any other relevant results in the manuscript. 
...
As illustrated in Fig.~\ref{fig:NN_table}, we see that the MSM tends to predict higher free energies for \rood{all macrostates relative to the H state compared to the NN model}.

\noteg{The line above is replacing the following: ``As illustrated in Fig.~\ref{fig:NN_table}, we see that the MSM tends to predict higher free energies for the out-of-register shifted macrostates 5S2, 3S2, 5S4, and 3S4 relative to the H state compared to the NN model. The NN and MSM predictions for the frayed state F4 are in good agreement.''}


\noteg{Do we still want to include these calculations or just say that it was wrong?}

Our back-of-the-envelope calculations yield a similar result based on the $\Delta F$ values reported in Fig.~3. In brief, assuming:
...
\noteg{Added sentence starting with ``Indeed, the sequence...'' Not totally sure about this, but I don't think we need to remove all references to all-or-nothing}
...
Updated and confirmed. \noteg{Pick a TOC that will fit in those dimensions}
